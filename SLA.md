...
|   Термин      | Значение      |
| ------------- | ------------- |
| Клиент  | Предприятие, подразделение предприятия или сотрудник предприятия, получающие Услуги от Поставщика услуг.  |
| Пользователь  | Сотрудник Клиента, ответственный за эксплуатацию оборудования и ПО, 
находящегося на обслуживании. |
| Поставщик услуг | Предприятие, подразделение предприятия или сотрудник предприятия, предоставляющие Услуги Клиенту.  |
| Эскалация | Передача решения инцидента или проблемы на более полномочный уровень, в случае исчерпания всех способов решения на текущем уровне. |
| Заявка| Зарегистрированное и классифицированное Обращение Пользователя, принятое к исполнению.  |
| Время реакции на устранение проблемы | Время с момента получения уведомления до инициирования действий по устранению проблемы.  |
| Время отклика сервера  | Время, которое требуется серверу для обработки запроса от клиента и отправки ответа обратно клиенту.  |
| Время восстановления после сбоя веб-сервера | интервал времени, который требуется для восстановления нормальной работы сервера после его отказа или сбоя. |
| Отчетный период | Один календарный месяц, в котором оказывались услуги.  |
...
По требованию Клиента Поставщик услуг должен предоставлять отчеты выполненных работ за отчетный период.
...
- Список полученных и удовлетворенных запросов на изменени
...
4.5. Перечень услуг по сопровождению.

Все услуги разделены на следующие категории:


- Действия, выполняемые по инициативе Поставщика (S1)

- Действия, выполняемые Поставщиком по запросу пользователей, включая запросы на изменения, в рамках постоянной части стоимости договора (S2)

- Действия, выполняемые Поставщиком по запросу пользователей, включая запросы на изменения, в рамках переменной части стоимости договора (S3)

- Действия, выполняемые Клиентом  самостоятельно, без согласования с Поставщиком (Z1)

- Действия, выполняемые Клиентом самостоятельно, после согласования  с Поставщиком (Z2)

| Услуга                 | Тип | Срок |
|------------------|----------------------|-------------------|
| Диагностика неисправностей | Z1,S2               | Т1              |
| Установка системных обновлений (критические патчи, сервис-паки) и обновлений антивирусных баз        | Z2,S1       | Т2                       |
| Изменение настроек ПО | Z2,S2                       | T2   |
|  Резервное копирование | Z2,S1  | Т2                        |
| Восстановление данных и системы при сбоях | Z2,S1              | Т2                        |
| Восстановление данных по запросу | Z2,S2 | Т2                        |
|  Замена аппаратных серверов при восстановлении после сбоев             | S1                       | Т1                       |
|   Замена аппаратных серверов (апгрейд) | S3              | Т3                        |
| Обновление серверного ПО кроме установки патчей и сервис-паков | S3 | Т3                        |
|  Мониторинг             |Z1,S1                     | Т3                      |
...
Срок удовлетворения запроса T3  и всех прочих запросов, не перечисленных в п. 4.5 – по договоренности между Поставщиком и Клиентом.

4.6. Приориеты

| Влияние на работу пользователей | Заголовок столбца 2 | Заголовок столбца 3 | Количество вовлеченных пользователей |
|-------------------|Один|Менее 10|Более 10|
| Значение 1, ячейка 1  | Значение 1, ячейка 2  | Значение 1, ячейка 3  | Значение 1, ячейка 4  |
| Значение 2, ячейка 1  | Значение 2, ячейка 2  | Значение 2, ячейка 3  | Значение 2, ячейка 4  |
| Значение 3, ячейка 1  | Значение 3, ячейка 2  | Значение 3, ячейка 3  | Значение 3, ячейка 4  |
